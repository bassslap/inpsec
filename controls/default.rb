...
control "timezone-ntp-config" do
  ...
  only_if("Supported on Linux only") do
    os.linux?
  end

  # Check timezone is set to America/New_York using timedatectl (systemd systems)
  if command("timedatectl").exist?
  ...
  if file(chrony_conf_path).exist?
    describe file(chrony_conf_path) do
      ntp_servers.each do |server|
        # Check if server appears in the config file content
